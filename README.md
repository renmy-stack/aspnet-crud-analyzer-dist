...
6. **「🔀 フロー図」**を押すと、データフローの**ノードグラフ**がブラウザで開きます
   - 形で種別を区別: **処理=四角 / テーブル=シリンダー / その他=六角形**
   - **エッジ色＝CRUD種別**（新規=緑 / 更新=橙 / 参照=灰）、**ノードの大きさ＝重要度**
   - ノードをクリックで下流=青／上流=赤をハイライト、🔎**検索**で絞り込み
   - 独立した処理の**塊を分けて表示**、レイアウトは**エッジ交差を自動で最小化**
...
- **データフローのノードグラフ可視化**（形で種別表示・CRUD色分け・上流/下流ハイライト・検索・交差最小化・参照マスタ自動判定・ドラッグ配置保存）
