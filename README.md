...
6. **「🔀 フロー図」**を押すと、データフローの**ノードグラフ**がブラウザで開きます
   - 形で種別を区別: **処理=四角 / テーブル=シリンダー / その他=六角形**、上部に**層見出し**（画面/テーブル/バッチ）
   - **エッジ色＝CRUD種別**（新規=緑 / 更新=橙 / 参照=灰）、**ノードの大きさ＝重要度**
   - クリックで下流=青／上流=赤、**右に詳細パネル**（役割・CRUD内訳・つながり）、🔎**検索**
   - **ダブルクリックでその上下流だけにフォーカス**、独立した処理の**塊を分けて表示**
   - **賢い自動配置**（交差最小化＋直線化＋長辺の中継整流）＋**直交線**表示
   - ノードを**ドラッグで自由移動**＆配置を保存。外部依存ゼロの単一HTML（ネット不要）
   - 操作: **ミニマップ**・全体表示/ズーム・**検索**・パネルから**ノードへジャンプ**・ホバー概要・
     **右クリックメニュー**・**Shift で複数選択＆整列**・**Undo/Redo(Ctrl+Z/Y)**・**PNG出力**・`?`で操作ガイド
